...
bool check_login() {
	const char * cmd =
			"wget --post-data= --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/check_login\"";
	int ret = 0;
...
int _get_jobs_http(int * jobs) {
	...
	char buf[BUFFER_SIZE];
	const char * cmd =
			"wget --post-data=\"oj_lang_set=%s&max_running=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/get_job\"";
...
bool _check_out_http(int solution_id, int result) {
	login();
	const char * cmd =
			"wget --post-data=\"sid=%d&result=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/checkout\"";
	int ret = 0;
