...
static char pass_data[BUFFER_SIZE];
...
int compare_zoj(const char *file1, const char *file2) {
	printf("ä½¿ç”¨æµ™å¤§è¯„æµ‹ç¨‹åºè¯„æµ‹=.=\n");
	int ret = OJ_AC;
	int c1, c2;
	FILE * f1, *f2;
	f1 = fopen(file1, "re");
...
/*
è¯„æµ‹æœºç™»å½•æ£€æŸ¥
æ•°æ®åœ°å€ base/check_login
å‚æ•°
	checklogin = 1(å›ºå®šå€¼)
è¿”å›ž
	0 æœªç™»å½•
	1 ç™»å½•
*/
bool check_login() {
	const char * cmd =
			" wget --post-data=\"checklogin=1\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/check_login\"";
	int ret = 0;
...
void login() {
	if (!check_login()) {
		const char * cmd =
				"wget --post-data=\"username=%s&password=%s\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/login\"";
...
void update_solution(int solution_id, int result, int time, int memory, int sim,int sim_s_id, double pass_rate,char* pass_detail = "") {
	if (result == OJ_TL && memory == 0)
		result = OJ_ML;
	const char * cmd =
			" wget --post-data=\"sid=%d&result=%d&time=%d&memory=%d&sim=%d&simid=%d&pass_rate=%f&pass_detail=%s\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/update_solution\"";
	FILE * fjobs = read_cmd_output(cmd, solution_id, result, time, memory, sim,sim_s_id, pass_rate,pass_detail, http_baseurl);
	//fscanf(fjobs,"%d",&ret);
	pclose(fjobs);
}
...
void addceinfo(int solution_id) {
	...
	fprintf(ce, "sid=%d&info=%s", solution_id, ceinfo_encode);
	fclose(ce);
	free(ceinfo_encode);

	const char * cmd =
			" wget --post-file=\"ce.post\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/upload_info\"";
	FILE * fjobs = read_cmd_output(cmd, http_baseurl);
	//fscanf(fjobs,"%d",&ret);
	pclose(fjobs);
}
...
/*
å‘é€REæ•°æ®ï¼Œç”¨æˆ·è¾“å‡ºï¼Œä¸åŒ
æ•°æ®åœ°å€
	base/upload_info
å‚æ•°
	sid info
*/
void addreinfo_http(int solution_id, const char * filename) {
	...
	fprintf(re, "sid=%d&info=%s", solution_id, reinfo_encode);
	fclose(re);
	free(reinfo_encode);

	const char * cmd =
			" wget --post-file=\"re.post\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/upload_info\"";
	FILE * fjobs = read_cmd_output(cmd, http_baseurl);
	//fscanf(fjobs,"%d",&ret);
	pclose(fjobs);
...
void addreinfo(int solution_id) {
		addreinfo_http(solution_id, "error.out");
}

void adddiffinfo(int solution_id) {
		addreinfo_http(solution_id, "diff.out");
}
void addcustomout(int solution_id) {
		addreinfo_http(solution_id, "user.out");
}


/*
æ›´æ–°ç”¨æˆ·ACé¢˜æ•° åºŸå¼ƒ
æ•°æ®åœ°å€
å‚æ•°
*/
...
/*
èŽ·å–ç”¨æˆ·è¾“å…¥,å®žæ—¶ç¼–è¯‘é¢„ç•™æŽ¥å£
æ•°æ®åœ°å€
	base/user_code
å‚æ•°
	sid æäº¤ç¼–å·
...
void get_solution(int solution_id, char * work_dir, int lang) {
	...
	const char * cmd2 =
			"wget --post-data=\"sid=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O %s \"%s/user_code\"";
...
void get_custominput(int solution_id, char * work_dir) {
	...
	const char * cmd2 =
			"wget --post-data=\"getcustominput=1&sid=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O %s \"%s/user_test_data\"";
	FILE * pout = read_cmd_output(cmd2, solution_id, src_pth, http_baseurl);
...
void get_solution_info(int solution_id, int & p_id, char * user_id,int & lang) {
	...
	const char * cmd =
			"wget --post-data=\"sid=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/user_code_info\"";
...
void get_problem_info(int p_id, int & time_lmt, int & mem_lmt, int & isspj) {
	const char * cmd =
			"wget --post-data=\"pid=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/problem\"";
	FILE * pout = read_cmd_output(cmd, p_id, http_baseurl);
	fscanf(pout, "%d", &time_lmt);
//	time_lmt=time_lmt/1000;
	fscanf(pout, "%d", &mem_lmt);
	fscanf(pout, "%d", &isspj);
	pclose(pout);

	if(time_lmt<=0) time_lmt=1000;
...
//      char java_p1[BUFFER_SIZE], java_p2[BUFFER_SIZE];
	...
	// time limit
	if (oi_mode)
		LIM.rlim_cur = time_lmt*0.0001/cpu_compensation + 1;//修改最大运行时间
	else
		LIM.rlim_cur = (time_lmt*0.0001/cpu_compensation  - usedtime / 1000) + 1;//修改最大运行时间
	LIM.rlim_max = LIM.rlim_cur;
	if(DEBUG) printf("LIM_CPU=%d",(int)(LIM.rlim_cur));
	setrlimit(RLIMIT_CPU, &LIM);
	alarm(0);
	alarm(time_lmt*0.0001 * 5 / cpu_compensation);
...
void judge_solution(int & ACflg, int & usedtime, int time_lmt, int isspj,
	...
	if (!oi_mode)
		num_of_test = 1.0;
	if (ACflg == OJ_AC
			&& usedtime > time_lmt * 1 * (use_max_time ? 1 : num_of_test))
		ACflg = OJ_TL;
...
void watch_solution(pid_t pidApp, char * infile, int & ACflg, int isspj,
	...
	if (DEBUG){
		...
		printf("watch_solution:time=%d,mem=%d",time_lmt,mem_lmt);
	}
...
//jvm gc ask VM before need,so used kernel page fault times and page size
    if (lang == 3 || lang == 7 || lang == 9 || lang == 13 || lang == 14 || lang == 16 || lang == 17) {
		...
		if (WIFSIGNALED(status)) {
			 ...
			 *
			 *  psignal(int sig, char *s)ï¼Œlike perror(char *s)ï¼Œprint out s, with error msg from system of sig
...
int get_test_file(char* work_dir, int p_id) {
	...
	int ret = 0;
	const char * cmd =
			" wget --post-data=\"pid=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/test_data_list\"";
	FILE * fjobs = read_cmd_output(cmd, p_id, http_baseurl);
	while (fgets(filename, BUFFER_SIZE - 1, fjobs) != NULL) {
		sscanf(filename, "%s", filename);
		if(!data_list_has(filename))
		...
		const char * check_file_cmd =
				" wget --post-data=\"pid=%d&filename=%s\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O -  \"%s/get_test_data_info\"";
		...
		if (access(localfile, 0) == -1 || local_date < remote_date) {
			...
			const char * cmd2 =
					" wget --post-data=\"pid=%d&filename=%s\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O \"%s\"  \"%s/get_test_data\"";
...
int main(int argc, char** argv) {
	...
	for (; (oi_mode || ACflg == OJ_AC|| ACflg == OJ_PE) && (dirp = readdir(dp)) != NULL;) {
		...
		if(http_download&&(!data_list_has(dirp->d_name)))
			continue;
