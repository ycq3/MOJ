    ...
    <div class="container card-deck-wrapper text-left">
        <div class="card-deck">
            <div id="pro" class="card overflow">
                <div id="head" class="text-center card-header">
                    ...
                    <div class="card-text">
                        <div>
                            <small>Time Limit: {{problemData.time}}/{{problemData.other_time}} MS (C/Others) &emsp;
                            </small>
                            <small> Memory Limit: {{problemData.memory}}/{{problemData.other_memory}} K (C/Others)
                            </small>
                        </div>
                    </div>
                </div>
            ...
            <div class="card col-sm-8 col-12" v-if="editorShow">
                <div id="edit" class="col-12 float-right">
                    <div style="background-color: rgba(0, 0, 0, 0.03)" class="form-inline">
                        ...
                        <div class="col">
                            <button type="button" class="close float-right form-control" aria-label="Close"
                                    v-on:click="show()">
        ...
        <div class="card-footer">
            <div class="col text-center">
                ...
                <button id="historyBtn" class="btn btn-success" style="width: 69px"  v-on:click="viewHistory">{{showHistory}}</button>
            </div>
        </div>
                        ...
                        <table class="table text-center">
                            ...
                            <transition-group name="list" tag="tbody">
                                <tr v-for="item in status_list" v-bind:key="item['runid']" id="status_list" v-bind:class="item['result']==6?'table-danger':item['result']==4?'table-success':'table-info'+' list-item'" >
                                    ...
                                    <td>
                                        <router-link v-bind:to="'/examViewCode/'+item['runid']" tag="a" target="_blank">{{languageSelect[item['language']]}}</router-link>
                                    </td>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //加载数据
            load() {//获取问题详情
                axios.post('/exam/show/' + this.e_id+'/problem/'+this.p_id).then(response => {
            ...
            submit() {//提交代码
                if (this.globalLoginData.islogin == false) {//提交前登陆检查
                    ...
                    axios.post('/exam/submit/' + this.e_id + '/problem/' + this.p_id, {
            ...
            viewHistory() {//查看历史记录前检查登陆
                if (this.globalLoginData.islogin == false) {
                    ...
                    $("#subHistory").modal();
                    axios.post('/exam/status/'+ this.e_id+'/problem/'+this.p_id).then(response=>{
